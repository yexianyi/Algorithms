package com.yxy.algorithms.array.non.decreasing;

/**
 * Given an array with n integers, your task is to check if it could become non-decreasing by modifying at most 1
 * element.
 * 
 * We define an array is non-decreasing if array[i] <= array[i + 1] holds for every i (1 <= i < n).
 * 
 * Example 1: Input: [4,2,3] Output: True Explanation: You could modify the first 4 to 1 to get a non-decreasing array.
 * Example 2: Input: [4,2,1] Output: False Explanation: You can't get a non-decreasing array by modify at most one
 * element. Note: The n belongs to [1, 10,000].
 * 
 * @author yexianyi 2019/01/17
 * Copy from:
 * https://blog.csdn.net/u011809767/article/details/77621955
 */
public class Solution {

    public static boolean checkPossibility(int[] nums) {
        boolean modified = false;
        for (int i = 1; i < nums.length; i++) {
            if ( nums[i - 1] > nums[i] ) {
                if (modified)
                    return false;
                if (i - 2 < 0 || nums[i - 2] <= nums[i]) {
                    nums[i - 1] = nums[i];
                } else {
                    nums[i] = nums[i - 1];
                }
                modified = true;
            }
        }
        return true;

    }

    public static void main(String[] args) {
        System.out.println(checkPossibility(new int[] { 3, 4, 2, 3 }));
        System.out.println(checkPossibility(new int[] { 2, 3, 3, 2, 4 }));
        System.out.println(checkPossibility(new int[] { 1, 3, 2 }));
        System.out.println(checkPossibility(new int[] { 4, 2, 3 }));
        System.out.println(checkPossibility(new int[] { 4, 2, 1 }));
    }

}
