package com.yxy.algorithms.array.task.scheduler;
...
public class Solution {
    ...
    public static int leastInterval(char[] tasks, int n) {
        ...
        // frequency from smaller to bigger
        Arrays.sort(frequencies);
        
        // get the biggest frequencies num
        int freqMax = frequencies[25] ; 
        
        int idleTime = (freqMax - 1) * n ;
        
        for(int i=24; i>=0 && idleTime >= 0; i--) {
            idleTime = idleTime - Math.min(freqMax - 1, frequencies[i]) ;
        }
